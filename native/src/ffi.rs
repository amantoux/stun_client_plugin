...
pub extern "C" fn get_xor_mapped_address(
...
impl From<&CDuration> for Duration {
    fn from(c_duration: &CDuration) -> Self {
        let secs: u64 = c_duration.secs.try_into().expect(&format!(
            "Could not convert CDuration.seconds to Duration.seconds : {}",
            c_duration.secs
        ));
        let nanos: u32 = c_duration.nanos.try_into().expect(&format!(
